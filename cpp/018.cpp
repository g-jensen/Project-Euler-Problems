...
int output[16384] = {0};
int index = 0;
void getPathValue(int tree[15][15], size_t i, size_t k, size_t acc) {
    if (i < 14 && k < 14) {
        i++;
        int l = tree[i][k];
        int r = tree[i][k+1];
        getPathValue(tree,i,k,acc+l);
        getPathValue(tree,i,k+1,acc+r);
    } else {
        output[index] = acc;
        index++;
    }
...
int main () {
    ...
    getPathValue(tree,0,0,75);
    std::cout << *std::max_element(output,output+16384) << std::endl;
}
